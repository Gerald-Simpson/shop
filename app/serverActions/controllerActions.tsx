...
import { fetchBasket, compareBasket, fetchBasketItem, fetchStockVariantWithId } from './viewActions.tsx';
import {
    basketItem,
    stockDbItem,
    stockVariantItem,
...
export async function reduceStock(cookieId: string) {
    ...
    basket.forEach(async (basketObj: basketItem) => {
        try {
            ...
            var stockVarItem: stockVariant = await fetchStockVariantWithId(basketObj.variantId)
            // Check that there is enough in stock
            if (stockVarItem.stock < basketObj.quantity) {
                console.error('Quantity in basket greater than stock in reduceStock function.')
                throw new Error('Quantity in basket greater than stock in reduceStock function.')
            } else {
            ...
            console.error(
                err + ': Failed fetching and updating stock variants in reduceStock function.',
            );
            throw new Error(
                err + ': Failed fetching and updating stock variants in reduceStock function.',
            );
...
export async function removeOutOfStock(cookieId: string) {
    ...
    try {
        let outStock: stockListItem[] = await compareBasket(cookieId).then(
            (allStock) => allStock.outStock,
        );
        outStock.forEach(async (variant) => {
            await decrementBasketByVariantId(cookieId, variant.variantId, variant.quantity)
        }, function() {
            revalidateTag('basketTag')
        })
    } catch (err) {
